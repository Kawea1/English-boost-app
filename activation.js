...
const ActivationUI = {
    ...
    bindDialogEvents() {
        ...
        if (trialBtn) {
            // 确保按钮可点击
            trialBtn.style.pointerEvents = 'auto';
            trialBtn.style.cursor = 'pointer';
            trialBtn.style.zIndex = '100';
            
            // 使用多种事件确保响应
            const handleTrialStart = (e) => {
                e.preventDefault();
                e.stopPropagation();
                ...
                // 按钮反馈动画
                trialBtn.style.transform = 'scale(0.95)';
                setTimeout(() => {
                ...
                if (result.success) {
                    ...
                    const errorEl = document.getElementById('activation-error');
                    const errorText = document.getElementById('error-text');
                    if (errorText) errorText.textContent = result.message || '开始试用失败，请稍后重试';
                    errorEl?.classList.add('show');
    ...
    bindSoundEffects() {
        // 试用按钮悬浮音效
        const trialBtn = document.getElementById('start-trial-btn');
        trialBtn?.addEventListener('mouseenter', () => this.playSound('hover'));
    ...
    startSmartRecommendation() {
        ...
        if (hour >= 5 && hour < 9) {
            recommendations = [
                '早起学习效率最高！建议从词汇开始，每天10个新单词 📚',
                '研究表明早晨记忆力最佳，现在开始背单词事半功倍！',
                '早安！开启30天试用，让学习成为每天的好习惯 ☀️'
            ];
        } else if (hour >= 9 && hour < 12) {
            recommendations = [
                '上午精力充沛，适合精听训练，提升听力理解能力 🎧',
                '抓住上午的黄金时间，每天15分钟精听练习效果显著！',
                '现在开始试用，解锁全部听力资源，随时随地练习 📱'
            ];
        } else if (hour >= 12 && hour < 14) {
            recommendations = [
                '午休时间！可以轻松浏览词汇卡片，碎片化学习 ☕',
                '饭后10分钟复习，帮助巩固上午所学内容 💪',
                '开启试用，支持离线学习，地铁公交都能用！'
            ];
        ...
        setTimeout(() => {
            ...
            const typeWriter = () => {
                if (i < recommendation.length) {
                    textEl.textContent = recommendation.substring(0, i + 1);
                    i++;
    ...
    injectStyles() {
        ...
        styles.textContent = `
            ...
            .sparkle:nth-child(1) { top: -5px; left: 20%; }
            .sparkle:nth-child(2) { top: 30%; right: -10px; }
            .sparkle:nth-child(3) { bottom: 5px; left: -5px; }
